...
class learningStat():
    ...
    def MAE(self):
        ...
        '''
        if self.numSamples > 0: 
            return self.errorAngleSum/self.numSamples 
        else:   
            return None
    def accuracy(self):
        '''
    ...
    def displayString(self):
        ...
        if loss is None:    # no stats available
            ...
            if minloss is None: # accuracy and minloss stats is not available
                return 'loss = %-11.5g    mae = %-8.5g'%(loss, mae)
            else:   # accuracy is not available but minloss is available
                return 'loss = %-11.5g (min = %-11.5g)    mae = %-8.5g'%(loss, minloss, mae)
        else:
            if minloss is None and maxAccuracy is None: # minloss and maxAccuracy is available
                return 'loss = %-11.5g        %-11s     accuracy = %-8.5g        %-8s     mae = %-8.5g'%(loss, ' ', accuracy, ' ', mae)
            else:   # all stats are available
                return 'loss = %-11.5g (min = %-11.5g)    accuracy = %-8.5g (max = %-8.5g)    mae = %-8.5g'%(loss, minloss, accuracy, maxAccuracy, mae)
